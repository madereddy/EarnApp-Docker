...
# --------------------------
if [[ ! -x "$BIN_PATH" ]]; then
    log "EarnApp binary not found, installing..."

    TMP_INSTALL="/tmp/earnapp.sh"
    curl -fsSL "$INSTALLER_URL" -o "$TMP_INSTALL"

    if [[ -n "${EARNAPP_VERSION:-}" ]]; then
        log "Patching installer to install version $EARNAPP_VERSION..."
        sed -i "s/^VERSION=\"[^\"]*\"/VERSION=\"$EARNAPP_VERSION\"/" "$TMP_INSTALL"
    fi

    # Run installer silently unless debug is enabled
    if [[ "$DEBUG_MODE" == "1" ]]; then
...
# --------------------------
# Ensure directories & status file
# --------------------------
...
if [[ -n "${EARNAPP_UUID:-}" ]]; then
    ...
    log "Using UUID from environment"
elif [[ ! -f "$CONFIG_DIR/uuid" ]]; then
    warn "No UUID found — will be generated by EarnApp"
fi
...
for i in $(seq 1 $MAX_ATTEMPTS); do
    ...
    log "Waiting for EarnApp registration (attempt $i/$MAX_ATTEMPTS)..."
    "$BIN_PATH" stop 2>/dev/null || true
    sleep 1
    "$BIN_PATH" start 2>/dev/null || true
    sleep "$BACKOFF"
...
if [[ "$DEVICE_ID" == "unknown" ]]; then
    warn "UUID not yet available; registration may still be in progress."
else
...
while true; do
    ...
    if [[ $RUN_DURATION -gt 60 ]]; then
        ...
        warn "EarnApp crashed after ${RUN_DURATION}s, backing off ${BACKOFF}s..."
        sleep "$BACKOFF"
        BACKOFF=$((BACKOFF * 2))
